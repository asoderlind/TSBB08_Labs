...
[X1, Y1] = meshgrid(1:cols, 1:rows); % in-image grid
[X2, Y2] = meshgrid(1:cols, 1:rows); % out-image grid
...
errimage = (backrotIm-Im).*(backrotIm-Im);
nanvals = isnan(errimage);
errimage(nanvals) = 0;
errenergy = sum(sum(errimage))
